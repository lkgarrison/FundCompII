...
void C4Board::display() {
	for(int h = 0; h < nCol; h++) cout << " " << h << "  "; // print col #'s
	cout << endl; // newline

	// iterate accross hole board, from top left to bottom right
	for(int i = board[0].getMaxDiscs() - 1; i >= 0; i--) {	// top of cols, downward
		for(int j = 0; j < nCol; j++) {		// for each disc space in each row
			cout << " " << board[j].getDisc(i);
			if(j < nCol - 1) cout << " |";			// only place |'s btwn columns
		}
		cout << endl;
	}
}
...
void C4Board::play() {
	...
	while(1) {	
		...
		if(colDrop == -1) {   	// user ended game
			...
			board[colDrop].addDisc(curPiece);  				// X for p1, 0 for p2
