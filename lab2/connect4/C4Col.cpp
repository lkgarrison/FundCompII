...
// since implementing the overloaded += operator, this becomes an internal method
...
// overload += operator so that adding a disc can be done by C4Colobj += boardPiece
C4Col C4Col::operator+=(char newDisc) {
	addDisc(newDisc);		// call the addDisc method that is already written
